...
public class Ex1 {
     ...
     * или текст вместо числа), должно быть брошено исключение MyArrayDataException с детализацие, в какой именно
     * ячейке лежат неверные данные.
     * 3. В методе main() вызвать полученный метод, обработать возможные исключения
     * MyArraSizeException и MyArrayDataException и вывести результат расчётов.
     */
    ...
    public static void main(String[] args) {
        for (int i = 0; i < 10; i++) {
            System.out.println("Генерация " + (i + 1));
            processArray();
        }
    }

    public static void processArray() {
        // запуск генерации обработки исключений
        try {
            System.out.printf("Сумма всех элементов: %d", processArrayInternal(generateArray()));
    ...
    private static int parseElement(String stroka, int i, int j) throws MyArrayDataException {
        try {
            ...
            throw new MyArrayDataException("Некорректный формат ", i, j);
        }
    }
